...
class ComprehensiveMultiDirectoryTest < Minitest::Test
  ...
  def test_comprehensive_multi_directory_multi_format_merging
    ...
    assert_equal true, db[:ssl], "SSL should be from local JSON"
    ...
    assert_equal false, app[:debug], "Debug should be from global YAML (highest priority)"
    assert_equal 'debug', app[:log_level], "Log level should be from environment JSON"
    assert_equal true, app[:local_override], "Local override should be from local YAML"
    assert_equal 'development', app[:environment], "Environment should be from local JSON"
    
    # Test features array merging
    features = config[:features]
    assert_equal true, features[:feature_a], "Feature A should be from global YAML"
    assert_equal false, features[:feature_b], "Feature B should be from global YAML (highest priority)"
    assert_equal true, features[:feature_c], "Feature C should be from environment JSON"
  ...
  def test_directory_order_matters
    ...
    assert_equal false, config[:app][:debug], "Local debug setting should override others"
  end
